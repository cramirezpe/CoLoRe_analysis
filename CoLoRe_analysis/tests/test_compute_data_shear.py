...
class TestDataTreatment(unittest.TestCase):
    ...
    def tearDown(self):
        data_treated_path = self.analysis_path + '/data_treated'
        if os.path.isdir(data_treated_path):
            rmtree(data_treated_path)
    
    def test_data_treatment_without_bins(self):
        data_treatment(self.sim_path, source=2, do_cls=True, do_kappa=True, output_path=self.analysis_path + f'/data_treated/source_2')

        a = np.loadtxt(self.analysis_path + f'/data_treated/source_2/mp_E.dat')
        b = np.loadtxt(self.analysis_path + f'/data_treated/source_2/cld_dd.dat')
        d = np.loadtxt(self.analysis_path + f'/data_treated/source_2/cld_kk.dat')
    ...
    def test_data_treatment_with_bins_2(self):
        ...
        data_treatment(self.sim_path, source=2, minz=2, maxz=2.005, output_path = self.analysis_path + f'/data_treated/binned/200_2005/source_2', do_cls=False, do_kappa=False)

        a = np.loadtxt(self.analysis_path + f'/data_treated/binned/200_2005/source_2/mp_E.dat')
