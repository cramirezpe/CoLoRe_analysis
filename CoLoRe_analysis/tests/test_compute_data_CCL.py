...
from CoLoRe_analysis import sims_reader, compute_data_CCL
from CoLoRe_analysis.tests import test_ccl_reader
...
class TestComputeDataCCL(unittest.TestCase):
    ...
    def test_normal_output_data_anafast(self, mocked_print, mocked_time):
        ...
        compute_data_CCL.compute_data(self.sim_path, self.analysis_path, zbins=[0,0.15,1],code='anafast')
    ...
    @patch('CoLoRe_analysis.compute_data_CCL.datetime')
    @patch('builtins.print')
    def test_normal_output_no_lensing_anafast(self, mocked_print, mocked_time):
        with open(self.analysis_path+ '/all_data_anafast.pkl','rb') as input:
            target_values = pickle.load(input) 
        ...
        mocked_time.today.return_value = date(2020,1,1)
        mocked_time.side_effect = lambda *args, **kw: date(*args, **kw)
        compute_data_CCL.compute_data(self.sim_path, self.analysis_path, zbins=[0,0.15,1], skip_lensing=True, code='anafast')

        # values = ['pairs', 'shotnoise', 'nz_tot', 'z_nz', 'cl_dd_d', 'cl_dd_t', 'cl_dm_d', 'cl_dm_t', 'cl_md_d', 'cl_md_t', 'cl_mm_d', 'cl_mm_t']
        values = ['pairs', 'shotnoise', 'nz_tot', 'z_nz', 'cl_dd_d', 'cl_dd_t']
    ...
    def test_normal_output_data_namaster(self, mocked_print, mocked_time):
        ...
        mocked_time.today.return_value = date(2020,1,1)
        mocked_time.side_effect = lambda *args, **kw: date(*args, **kw)
        compute_data_CCL.compute_data(self.sim_path, self.analysis_path, zbins=[0, 0.15, 1], code='namaster')
    ...
    @patch('CoLoRe_analysis.compute_data_CCL.datetime')
    @patch('builtins.print')
    def test_normal_output_no_lensing_namaster(self, mocked_print, mocked_time):
        with open(self.analysis_path+ '/all_data_namaster.pkl','rb') as input:
            target_values = pickle.load(input) 
        ...
        mocked_time.today.return_value = date(2020,1,1)
        mocked_time.side_effect = lambda *args, **kw: date(*args, **kw)
        compute_data_CCL.compute_data(self.sim_path, self.analysis_path, zbins=[0, 0.15, 1], skip_lensing=True, code='namaster')

        names = ['pairs', 'shotnoise', 'nz_tot', 'z_nz', 'cl_dd_d']
...
class TestMainFunction(unittest.TestCase):
    ...
    @patch('CoLoRe_analysis.compute_data_CCL.compute_data', side_effect=test_ccl_reader.mock_compute_data)
